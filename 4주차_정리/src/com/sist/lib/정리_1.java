 ...
 *                        ====> 1) 선언
 *                              2) try~catch 사용
 *                        예) public void method() throws 예외처리클래스,예외처리클래스...(,로 구분해서 여러개 쓸수있다)
 *                           {                           --------------------- 순서가 없다
 *                                                       method호출시에 예상되는 에러를 체크
 *                           }
 *                           
 *                           ==> 메소드 호출
 ...
 *                                   }
 *                                }
 *                = 예외 임의 발생 : 문장 뒤에 throw (테스트용으로 사용)
 ...
 *                 브라우저, 오라클 ...
 *              => 파일읽기 / 쓰기
 *              => 프로그램안에 다른 프로그램을 동시에 동작할때
 *              => 자료구조 (메모리 관리)
 *              -----------> 직접 만들어서 제공 (라이브러리)
 *              -----------> 사용자 정의 클래스 + 라이브러리 => 조립
 *                                                     ----
 *    9장/10장 => 라이브러리 (자주쓰는 라이브러리 6개)                        
 *    ------------------
	...
	***	9. 문자열에 지정한 문자" "가 있으면 새로 지정한 문자" "로 바꿔서 출력한다.
  	...
  	***	10. 문자열에 지정한 범위에 속하는 문자열을 반환한다.(시작범위에 값은 포함하고, 끝나는 범위에 값은 포함하지않는다.)
		...
		12. 문자열에 소문자를 대문자로 변환한다.
		    ...
		    ** 오라클은 대소문자를 구분하지 않는다
		       (대소문자 구분은 실제 저장된 값) (코딩상에서는 구분하지 않음)
 ...
 *    LinkedList
 *    
 *    List : 순서가 존재(인덱스번호 0번부터 시작), 중복데이터 허용
