 ...
 *                -------- 프로세스
 *          쓰레드는 시분할 => CPU를 나눠서 작업을 하기때문에 속도가 향상된다
 ...
 *          생성
 *                        start()       run()            sleep()
 *           new Thread() =====> 대기상태 =====> Running상태 =====> 대기상태
 *                                                |
 *                                              Dead상태 => interrupt()
...
public class Server implements Runnable{
	...
	private final int PORT=3456; //조번호로 설정
	// 서버 가동 ==> 한 컴퓨터안에서 서버를 두개 실행할 수 없다
	public Server()
	{
		try
		{
			ss=new ServerSocket(PORT); // PORT=>default
			// bind() => listen()
			// PORT => 50명 한정
