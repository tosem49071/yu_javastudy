...
/*
 *    자바 클래스의 구성 => *** 객체지향 특성 (접근지정어, 캡슐화, 재사용, 수정, 추가)  // 기술면접 주로 이런거 나옴
 *    -------------    public / protected / default / private
 *                     1) 변수 2) 메소드 3) 생성자
 *                     1) 캡슐화 : 데이터 보호 => 모든 변수는 private
 *                     2) 재사용 : 변경해서 사용, 있는 그대로 사용
 *                               상속(is-a)   포함(has-a)
 *                     3) 수정 : 오버라이딩, 추가 : 오버로딩
 *    class ClassName
 *    {
 ...
 *                 => 소멸시점 : 객체=null 이거나 사용하지 않는 경우
 *                            -------------- 가비지 컬렉션 (메모리 회수)
 *                 => 가장 많이 사용되는 변수 (여러개를 저장할 수 있는 변수)
 *                 => 사용하는 방법
 *                    class A
 *                    {
 *                        int age; //0
 ...
 *                    A a=new A(); => age,name
 *                        ---
 *                      -- 객체, 인스턴스(메모리에 저장된 상태)      // 저 a를 인스턴스라고
 *                       => a.age
 *                       => a.name
 *                    A b=new A(); => age,name
 *                        ---
 ...
 *                           --------   
 *                           
 *                  b=a; => 저장된 데이터관리 위치가 변경이 될 수 있다(데이터 손실)        //그러면 b=100이여서 a만 수행
 *                  =========> 메모리 주소의 복구가 어렵다                          //한번바꾸면 복구 안됨
 *                  A c=200
 *                   -a-
 *                   100  => 100--------       
 *                   ---      ------age     
