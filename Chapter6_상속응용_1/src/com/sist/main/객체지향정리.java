 ...
 *     method도 메모리에 저장     //메소드가 저장되려면 구현부가 있어야함
 *     void method()
 *     {
 *        구현부가 있어야 저장이 된다
 *     }
 ...
 *                          웹구현 
 *        void method();     //사용자마다 구현하는게 다르기때문에 규정된것이 없음
 *    -----------------------
 *     Stack : LIFO          //나중에 들어온게 먼저빠져나가는 방식(Last In F~ O~)
 ...
 *    -----------------------
 *     Heap : 동적메모리 할당 (new)
 *            => 관리를 프로그래머가 함 => GC(자동메모리 회수) //=가비지컬렉션
 *            => 객체=null
 *     A.java   ==>      A.class  ===>  객체변수, 배열(참조변수값) 저장
 *         javac(컴파일시킴)         java
 *     class A => 인터프리터 (=한줄씩 번역해 나가는 것)
 *     {
 *        ---
 *        ---
 *        ---new    // 읽다가 new가 나오면 메모리 할당 
 *        ---
 *        ---
 *     }
 *    -----------------------
 ...
 *                s1.substring()
 *             정적메소드 : 동시에 작동할때 쓰는 메소드 (static=>자동저장)
 *                10=>"10", true=>"true"
 *                String.valueOf(데이터형)
 *                Math.random()
