 ...
 *    1. 재사용
 *       = 상속 => is-a
 *         기존의 클래스를 확장해서 사용        ex) public class MelonMusic extends Music{}
 *                    -------
 *                    extends
 *         => 자바는 단일 상속만 가능하다
 *                 -------
 *         class A
 *         class B extends A  // A클래스를 확장해서 사용하겠다
 *         class C extends JFrame ==> 라이브러리
 *         ----------------------------------
 *         class A
 *         class B
 *         class C extends A,B => 오류 (C언어에서는 사용 가능)
 *         C=A+B+알파
 *         
 *         class A
 *         class B extends A
 *         class C extends B   // 하나씩 단일상속
 *         ------------------
 *         
 ...
 *                        -------- int a,b,c
 *         A > B => 상속을 내리는 클래스가 크다        //데이터가 많다고 커지는건 아니다
 *         
 *               동물
 *                |
 *         ---------------
